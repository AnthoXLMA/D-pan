...
// NOUVEAU ACTIVE REPAIR MODAL --------- 20250905 -------------------------
import React, { useEffect, useState } from "react";
import { releaseEscrow, refundEscrow } from "./services/escrowService.js";
import { doc, onSnapshot } from "firebase/firestore";
import { db } from "./firebase";
...
export default function ActiveRepairModal({ report, solidaire, userPosition, onComplete }) {
  ...
  const handleRelease = async () => {
    if (paymentStatus !== "released") {
      toast.error("Le paiement n'a pas encore été reçu !");
      return;
    }
    setLoadingAction(true);
    await releaseEscrow(report.id, setPaymentStatus);
  ...
  const handleRefund = async () => {
    if (paymentStatus === "refunded") {
      toast.error("Le paiement a déjà été remboursé !");
      return;
    }
    const confirm = window.confirm(
      "⚠️ Êtes-vous sûr de vouloir annuler l'assistance et rembourser le paiement ?"
    );
    if (!confirm) return;
  ...
  const handleItineraire = () => {
    if (!report.latitude || !report.longitude) {
      toast.error("Position du sinistré inconnue !");
      return;
    }
  ...
  const handleContacter = () => {
    if (!report.ownerEmail) {
      toast.error("Contact du sinistré indisponible !");
      return;
    }
    // Placeholder pour chat ou mail
    window.location.href = `mailto:${report.ownerEmail}`;
