...
import { doc, updateDoc } from "firebase/firestore";
import { db } from "./firebase.js";
import { STRIPE_API_URL } from "./services/stripeFrontendService.js"; // ✅ importe l’URL réseau
...
export default function InProgressModal({
  ...
  const handleComplete = async () => {
    try {
      ...
      if (result?.success) {
        ...
        toast.error(`❌ Erreur libération paiement : ${result?.error || "inconnue"}`);
      }
  ...
  const handleCreateStripeAccount = async () => {
    ...
    try {
      ...
      // 1️⃣ Créer le compte Stripe (POST /api/stripe/create-account)
      const accountRes = await fetch(`${STRIPE_API_URL}/create-account`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      });
      ...
      if (!accountRes.ok) {
        ...
        throw new Error(`create-account ${accountRes.status} ${txt?.slice(0,120)}`);
      }
      ...
      // 3️⃣ Créer le lien d’onboarding (POST /api/stripe/create-account-link)
      const linkRes = await fetch(`${STRIPE_API_URL}/create-account-link`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!linkRes.ok) {
        ...
        throw new Error(`create-account-link ${linkRes.status} ${txt?.slice(0,120)}`);
      }
      ...
      // 4️⃣ Redirection vers Stripe pour compléter l’onboarding
      console.log("🔹 Redirection vers Stripe onboarding :", linkData.url);
      window.location.href = linkData.url;
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-11/12 animate-fade-in relative text-center">
        ...
        {!solidaire.stripeAccountId ? (
          <button
            onClick={handleCreateStripeAccount}
            disabled={loading}
            className="w-full px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white font-semibold rounded-xl shadow-lg hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 transform hover:-translate-y-1 mb-4 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? "Connexion Stripe..." : "Connecter mon compte Stripe pour recevoir le paiement"}
          </button>
        ) : (
          <p className="text-green-600 font-medium mt-2 mb-4">
            Votre compte Stripe est prêt. Vous recevrez automatiquement vos gains.
